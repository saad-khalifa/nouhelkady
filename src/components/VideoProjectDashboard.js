...
import '../styles/VideoProjectDashboard.css';

const VideoProjectDashboard = () => {
  ...
  const fetchProjects = async () => {
    try {
      ...
      console.log('Video Projects dashboard data:', response.data);
  ...
  const handleSaveEdit = async () => {
    try {
      ...
      const response = await axiosInstance.post(
        `/video-projects/${editingProject.id}`,
        formData,
        {
          headers: {
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmDelete) {
      try {
        await axiosInstance.delete(`/video-projects/${id}`);
  ...
  return (
    <Container className="video-project-dashboard mt-4">
      <h1 className="mb-4">لوحة تحكم مشاريع الفيديو</h1>
      ...
      <Modal show={showEditModal} onHide={handleCloseModal} size="lg" dir="rtl" className="edit-modal">
        <Modal.Header closeButton style={{ background: 'rgb(18,43,92)', color: 'white' }}>
          <Modal.Title>تعديل مشروع الفيديو</Modal.Title>
        </Modal.Header>
...
export default VideoProjectDashboard;