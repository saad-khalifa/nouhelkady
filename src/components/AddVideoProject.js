...
import { Upload, Image as ImageIcon, X, CheckCircle, Video } from 'lucide-react';
import axiosInstance from '../Axios/axiosInstance';  // استيراد axiosInstance
import '../styles/AddVideoProject.css';

const AddVideoProject = () => {
  ...
  const [url, setUrl] = useState(''); // إضافة الحالة الجديدة للرابط
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axiosInstance.post('/video-projects', formData, {  // استخدام axiosInstance هنا
      ...
      setImages([]);
      setImagePreviews([]);
      setUrl(''); // إعادة تعيين الرابط بعد الإرسال الناجح
  ...
  return (
    <div className="add-video-project-page">
      <Container className="add-video-project-container">
        <div className="page-header">
          <div className="header-icon">
            <Video size={40} />
          </div>
          <h1 className="page-title">إضافة مشروع مونتاج فيديو جديد</h1>
          <p className="page-subtitle">قم بإضافة مشروع جديد إلى معرض المونتاج والإنتاج</p>
        </div>
          ...
          <Card.Body>
            <Form onSubmit={handleSubmit}>
              ...
              <Form.Group className="form-group">
                ...
                <Form.Control style={{ color:'white' }}
                  type="text"
              ...
              <Form.Group className="form-group">
                <Form.Label style={{ color:'white' }} className="form-label">رابط المشروع (اختياري)</Form.Label>
                <Form.Control style={{ color:'white' }}
                  type="url"
                  className="form-input"
                  value={url}
                  onChange={(e) => setUrl(e.target.value)}
                  placeholder="أدخل رابط المشروع (إن وجد)"
                />
              </Form.Group>
